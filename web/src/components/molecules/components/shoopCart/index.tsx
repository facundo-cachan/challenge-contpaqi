...
const ShoppingCart = ({ className }: Props) => {
  ...
  const [favorites, setFavorites] =
    useRecoilState<FavoritesProps[]>(selectorFavorites);
  ...
  return (
    <div className={`flex flex-col ${className}`}>
      ...
      <Modal
        className={`mt-10 -mr-36 ${isVisible ? "block" : "hidden"} bg-blue`}
      >
        <ul className="flex flex-col">
          {favorites.map(({ id, title }: FavoritesProps) => (
            <li
              key={id}
              className="justify-between w-full p-0 align-middle border-b"
            >
        ...
        <Button onClick={() => setFavorites([])} primary>
          Remove All
        </Button>
      </Modal>
    </div>
